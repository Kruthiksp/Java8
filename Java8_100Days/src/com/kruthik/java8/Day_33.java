...
public class Day_33 {
	...
	public static void main(String[] args) {
		...
		Map<String, Double> groupByCity = orders.stream().filter(order -> order.getStatus().equals("DELIVERED"))
				.collect(Collectors.groupingBy(Order_Day25::getCity, Collectors.summingDouble(Order_Day25::getAmount)));

		Entry<String, Double> maxCity = groupByCity.entrySet().stream()
				.max(Map.Entry.<String, Double>comparingByValue()).get();

		System.out.println("City with max delivered sales: " + maxCity.getKey() + " -> " + maxCity.getValue());
